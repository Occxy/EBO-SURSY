...
var valid_field = 
	['Institut' ,'Gender' ,'Familly' ,'Commun name' ,'Date' ,'Number' ,'Square' ,'Commune' ,'Town' ,'Prefecture' ,'GPS'];
...
function search_N_identification_Recursif(/*localDB, */i) {
	...
	if (localStorage.getItem('web') === 'oui') {
		...
		var DB = new PouchDB(remote_couchdb + 'animals' + import_table + '_4' + debug);
	} else {
		var DB = new PouchDB('animals' + import_table + '_4' + debug);
	};
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		for(var rowCountContent = 0; rowCountContent < rowContent_length; rowCountContent++) {
		    			...
		    			if (rowCountContent == 10) {
		    				
		    				var coord = []
		    				coord = tab[i][rowCountContent].trim().split(",");
		    				
		    				var name_field = "Point_GPS_LAT";
		    				var rowContent = coord[0].trim();
		    				new_doc[name_field] = rowContent;
		    				
		    				var name_field = "Point_GPS_LONG";
		    				var rowContent = coord[1].trim();
		    				new_doc[name_field] = rowContent;
	...
	DB.get(id).then(function (doc) {
		...
		for(var rowCountContent = 0; rowCountContent < rowContent_length; rowCountContent++) {
			...
			if (rowCountContent == 10) {
				...
				var name_field = "Point_GPS_LAT";
				var rowContent = coord[0];
				doc[name_field] = rowContent;
				
				var name_field = "Point_GPS_LONG";
				var rowContent = coord[1];
				doc[name_field] = rowContent;
...
function synchronizeAnimals() {
	...
	if (localStorage.getItem('web') !== 'oui') {
		var localDB = new PouchDB('animals' + import_table +'_4' + debug);
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var remoteDB = new PouchDB(remote_couchdb + 'animals' + import_table +'_4' + debug, {skip_setup: true});
