...
var valid_field = 
	['N_site', 'Pays', 'Prefecture', 'Sous-prefecture', 'Ville/village', 'Site_capture', 'Environnement', 'Latitude', 'Lat_degre_dec', 'Longitude', 
	 'Long_degre_dec', 'Proximite_village_km', 'Proximite_source_m', 'Date_collecte', 'Equipe', 'ID_NI_faune', 'Espece_animale', 'Localisation'];
...
function fill_tab_datas_faune(file_line) {
	...
	if (file_line == lines_length - 1) {
		...
		search_N_identification_Recursif_Faune(/*DB,*/ lines_length-1, 0, -1, '', row);
	};
};

function search_N_identification_Recursif_Faune(/*localDB,*/ i, j, k, ID_NI_faune, row) {
	
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'faune' + import_table + debug);
	} else {
		var DB = new PouchDB('faune' + import_table + debug);
	};
	...
	if (i > 0) {
		...
		if (typeof tab[i] !== "undefined") {
			...
			ID_NI_faune = tab[i][15];
			
			DB.find({
		       	selector: {ID_NI_faune: ID_NI_faune}
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		//if (record_change_bat_capturees(result, rowContent_length)) {
		    			...
		    			search_N_identification_Recursif_Faune(i-1);
		    			//addEspeceInTableReferenceBatCapturees(i);
		    		...
		    		for(var rowCountContent = 0; rowCountContent < rowContent_length; rowCountContent++) {
						...
						if ((rowCountContent == 1) && (tab[i][rowCountContent] == "Guinee")) {
					...
					put_new_id_faune(new_doc/*, i*/);
					
					move();
					row++;
					
					search_N_identification_Recursif_Faune(/*localDB, */i-1);
...
function put_with_id_faune(id, i) {
...
function put_new_id_faune(doc/*, i*/) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'faune' + import_table + debug);
	} else {
...
function synchronizeFaune() {
	
	//alert('synchronizeBatNoninvasives');
	
	if (localStorage.getItem('web') !== 'oui') {
		var localDB = new PouchDB('faune' + import_table + debug);
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var remoteDB = new PouchDB(remote_couchdb + 'faune' + import_table + debug, {skip_setup: true});
