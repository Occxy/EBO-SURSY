...
var valid_field = 
	['Institut' ,'Gender' ,'Familly' ,'Commun name' ,'Date' ,'Number' ,'Square' ,'Commun' ,'Town' ,'Prefecture' ,'GPS'];
...
function search_N_identification_Recursif(/*localDB, */i) {
	...
	if (localStorage.getItem('web') === 'oui') {
		...
		var DB = new PouchDB(remote_couchdb + 'animals' + import_table + '_2' + debug);
	} else {
		var DB = new PouchDB('animals' + import_table + '_2' + debug);
	};
...
function synchronizeAnimals() {
	...
	if (localStorage.getItem('web') !== 'oui') {
		var localDB = new PouchDB('animals' + import_table +'_2' + debug);
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var remoteDB = new PouchDB(remote_couchdb + 'animals' + import_table +'_2' + debug, {skip_setup: true});
