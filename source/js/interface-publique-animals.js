...
DB.allDocs({  		
	...
	if (version_interface_publique == 'anglais') {
		...
		var resOut = alasql('SELECT pays, Point_GPS_LAT, Point_GPS_LONG, date, especes, animal, SUM(Individus_echantillonnes) AS Sampled_individuals, SUM(Plasma) AS Plasma, SUM(Serum) AS Serum, SUM(Blood_spot) AS Blood_spot, SUM(Coeur) AS Heart, SUM(Cerveau) AS Brain, SUM(Foie_rate) AS Liver_spleen, SUM(Glandes_salivaires) AS Salivary_glands, SUM(Poumons) AS Lungs, SUM(Reins) AS Kidneys, SUM(Intestins) AS Intestines, SUM(Urine) AS Urine, SUM(Ecouvillon_salivaire) AS Salivary_swab, SUM(Ecouvillon_rectal) AS Rectal_swab, SUM(Ecouvillon_nasal) AS Nasal_swab, SUM(Ectoparasites) AS Ectoparasites, SUM(Placenta) AS Placenta, SUM(Liquide_amniotique) AS Amniotic_fluid, SUM(Testicules) AS Testes, SUM(Lait) AS Milk, SUM(Foetus) AS Fetus, SUM(Guano) AS Guano, SUM(Longueur_bras) AS Arm_length, SUM(Longueur_avant_bras) AS Forearm_length, SUM(Longueur_corporelle) AS Body_length FROM ? GROUP BY pays, Point_GPS_LAT, Point_GPS_LONG, date, especes, animal', [tabOut] );
		console.log(resOut)
		
		interface_publique = JSON.stringify(resOut);
		
		CSV = 'photo;country;Point_GPS_LAT;Point_GPS_LONG;species;animal;' +
