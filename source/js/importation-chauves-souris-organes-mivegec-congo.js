	...
	if (lines_length > 1) {
		...
		if (fields_is_valid()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_chauves_souris_organes(line);
				};
...
function fill_tab_datas_chauves_souris_organes(file_line) {
...
function search_N_identification_Recursif(/*localDB, */i) {
	...
	if (localStorage.getItem('web') === 'oui') {
		...
		var DB = new PouchDB(remote_couchdb + 'chauves_souris_organes' + import_table + debug);
	} else {
		var DB = new PouchDB('chauves_souris_organes' + import_table + debug);
	};
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		if (record_exists(result)) {
		    			...
		    			put_with_id_chauves_souris_organes(id, i);
					...
					put_new_id_chauves_souris_organes(new_doc, i);
...
function put_with_id_chauves_souris_organes(id, i) {
...
function put_new_id_chauves_souris_organes(doc, i) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'chauves_souris_organes' + import_table + debug);
	} else {
...
function synchronizeAnimals() {
	...
	if (localStorage.getItem('web') !== 'oui') {
		var localDB = new PouchDB('chauves_souris_organes' + import_table + debug);
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var remoteDB = new PouchDB(remote_couchdb + 'chauves_souris_organes' + import_table + debug, {skip_setup: true});
