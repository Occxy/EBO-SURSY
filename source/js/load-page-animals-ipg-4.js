...
function modifier(table, option) {
	...
	var doc;
	if (option == 1) {
		doc = JSON.parse(localStorage.getItem('animalsIPG4TablesData'));
		...
		var id = localStorage.getItem('ID_animals' + table + '_4');
		...
		if (localStorage.getItem('web') === 'oui') {
			var remote_couchdb = localStorage.getItem('remote_couchdb');
			var DB = new PouchDB(remote_couchdb + 'animals' + table + '_4' + debug);
		} else {
			var DB = new PouchDB('animals' + table + '_4' + debug);
		};
		DB.allDocs({  		
				keys: [id],
			include_docs: true,
