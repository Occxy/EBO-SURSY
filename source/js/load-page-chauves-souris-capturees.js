...
function modifier(table, option) {
	...
	if (option == 1) {
		...
		//addValue('NumFilet', true);
		
		var element = document.getElementById('NumFilet');
		if (doc.NumFilet != '') {
			
			var value = '';
			
			var res = alasql('SELECT Filet FROM ?  WHERE Numfilet == "' + doc.NumFilet + '" GROUP BY Numfilet, Filet', [tabFilet] );
			...
			element.value = value;
			/*element.value = doc.NumFilet + ';' +
							doc.LongueurFilet + ';' +
							doc.HauteurFilet + ';' +
							doc.Lat_degre_decFilet + ';' +
							doc.LatitudeFilet + ';' +
							doc.Long_degre_decFilet + ';' +
							doc.LongitudeFilet;*/
			try {
				element.onchange();
			} catch(err) {
		...
		var Element = document.getElementById('Collier'); 
		if (doc.Collier === 'Oui') {
			Element.checked = true;
			Element.onclick();
		} else {
		...
		DB.allDocs({  		
			...
			if (typeof(JSON.stringify(result)) != "undefined"){  
				...
				var element = document.getElementById('NumFilet');
				/*element.value = result.rows[0].doc.NumFilet + ';' +
								result.rows[0].doc.LongueurFilet + ';' +
								result.rows[0].doc.HauteurFilet + ';' +
								result.rows[0].doc.Lat_degre_decFilet + ';' +
								result.rows[0].doc.LatitudeFilet + ';' +
								result.rows[0].doc.Long_degre_decFilet + ';' +
								result.rows[0].doc.LongitudeFilet 
				;*/
				
				var value = '';
				
				var res = alasql('SELECT Filet FROM ?  WHERE NumFilet == "' + result.rows[0].doc.NumFilet + '" GROUP BY NumFilet, Filet', [tabFilet] );
				...
				/*alert(result.rows[0].doc.NumFilet + ';' +
						result.rows[0].doc.LongueurFilet + ';' +
						result.rows[0].doc.HauteurFilet + ';' +
						result.rows[0].doc.Lat_degre_decFilet + ';' +
						result.rows[0].doc.LatitudeFilet + ';' +
				...
				var Element = document.getElementById('Collier'); 
				if (result.rows[0].doc.Collier === 'Oui') {
					Element.checked = true;
					Element.onclick();
				} else {
					Element.checked = false;
