...
DB.allDocs({  		
	...
	var resFeces = alasql('SELECT Lat_degre_dec, Latitude, Long_degre_dec, Longitude, Date, Famille_1, Genre_1, Espece_1, COUNT(*) AS Feces FROM ? WHERE Nbre_feces_par_tube <> 0 AND Nbre_feces_par_tube <> "" GROUP BY Lat_degre_dec, Latitude, Long_degre_dec, Longitude, Date, Famille_1, Genre_1, Espece_1', [tab] );
   		...
   		try {	
   			...
   			if (id !== newid) {
				...
				lastLat_degre_dec = row.Lat_degre_dec;
   				...
   				lastDate = row.Date;
				lastFamille = row.Famille_1;
   				lastGenre = row.Genre_1;
   				lastEspece = row.Espece_1;
