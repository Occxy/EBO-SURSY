...
var valid_field = 
	['N_espece_terrain', 'date_collecte', 'pays', 'lieu_code', 'lieu_principal',	
	 'lieu_detail', 'fraicheur', 'boite_labo', 'N_labo', 'temp1', 'temp2', 'espece_labo_mitDNA',
	 'espece_labo_12S', 'espece_consensus', 'individu_microsatellite', 'sous-espece', 'temp3', 'point_GPS', 
	 'GPS_corrige', 'observations_terrain', 'preveleur', 'autres_observations'];
	...
	if (lines_length > 1) {
		...
		if (fields_is_valid()) {
				...
				for (var line = 1; line < lines_length; line++) {
					...
					fill_tab_datas_grands_singes_antilopes(/*rowContent[0], */line);
				};
			/*} else {
...
function fill_tab_datas_grands_singes_antilopes(file_line) {
...
function search_N_identification_Recursif(/*localDB, */i) {
	...
	if (localStorage.getItem('web') === 'oui') {
		...
		var DB = new PouchDB(remote_couchdb + 'grands_singes_antilopes' + import_table + debug);
	} else {
		var DB = new PouchDB('grands_singes_antilopes' + import_table + debug);
	};
			...
			DB.find({
		    	...
		    	if (result.docs.length > 0) {
		    		...
		    		if (record_exists(result)) {
		    			...
		    			put_with_id_grands_singes_antilopes(id, i);
					...
					put_new_id_grands_singes_antilopes(new_doc, i);
...
function put_with_id_grands_singes_antilopes(id, i) {
...
function put_new_id_grands_singes_antilopes(doc, i) {
	if (localStorage.getItem('web') === 'oui') {
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var DB = new PouchDB(remote_couchdb + 'grands_singes_antilopes' + import_table + debug);
	} else {
...
function synchronizeGrandsSinges_Antilopes() {
	...
	if (localStorage.getItem('web') !== 'oui') {
		var localDB = new PouchDB('grands_singes_antilopes' + import_table + debug);
		var remote_couchdb = localStorage.getItem('remote_couchdb');
		var remoteDB = new PouchDB(remote_couchdb + 'grands_singes_antilopes' + import_table + debug, {skip_setup: true});
